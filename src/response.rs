    ...
    for response in responses.iter() {
            ...
            parser::head_bodystructure(&bodystructure_text)
        {
            ...
            match body_result {
                Ok((_, mut body)) => {
                    if set_header {
                        if let Some(header_text) = extract_header_text(&response) {
                            body.set_header(header_text.to_vec());
                        }
                    }
...
fn extract_header_text(data: &[u8]) -> Option<&[u8]> {
    if let Some(index) = data.iter().position(|&x| x == 0x0A) {
        if let Some(index2) = data.iter().rposition(|&x| x == 0x29) {
            if index >= index2 {
                return None;
            }
            return Some(&data[index + 1..index2]);
        }
    }
    None
}
